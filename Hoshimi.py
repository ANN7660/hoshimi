# === Version sans audio (fix complet VoiceClient pour Python 3.13) ===
import sys, types

# Création de faux modules audio pour que discord.py ne plante pas
fake_voice = types.ModuleType("discord.voice_client")

class VoiceClient:
    """Fausse classe VoiceClient (aucun son, juste pour compatibilité)"""
    def __init__(self, *args, **kwargs):
        pass

class VoiceProtocol:
    """Fausse classe VoiceProtocol (aucun son, juste pour compatibilité)"""
    pass

fake_voice.VoiceClient = VoiceClient
fake_voice.VoiceProtocol = VoiceProtocol

sys.modules["discord.voice_client"] = fake_voice
sys.modules["discord.player"] = types.ModuleType("discord.player")
sys.modules["discord.audio"] = types.ModuleType("discord.audio")

print("🚫 Mode sans audio activé : Discord Voice désactivé pour compatibilité Python 3.13\n")
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Game(name="Hoshimi | +help"),
...
async def on_member_join(member: discord.Member):
    ...
    if simple_channel:
        member_count = len(member.guild.members)
        message = (
            f"Bienvenue {member.mention} sur Hoshimi ! Nous sommes ravis de t'accueillir ! 🎉\n"
            f"Nous sommes désormais **{member_count}** membres sur Hoshimi ! ✨"
        )
...
async def on_member_join(member: discord.Member):
    ...
    if simple_channel:
        message = (
            f"{ARROW_EMOJI} Bienvenue {member.mention} sur Hoshimi ! Nous sommes ravis de t'accueillir ! 🎉\n"
            f"{ARROW_EMOJI} Nous sommes désormais **{member_count}** membres sur Hoshimi ! ✨"
        )
        await simple_channel.send(message)
...
async def help_command(ctx):
    embed = discord.Embed(
        title="🌌 Aide — Commandes de Hoshimi",
    ...
    embed.set_footer(text="✨ Hoshimi — by your starry bot 🌙")
    await ctx.send(embed=embed)
...
# ======== AJOUTS GPT (ne rien supprimer au-dessus) ========
# Ajout : aurole, createvoc, lock/unlock, auto-temp-voice, help stylisé
import asyncio, json, os

# Utilise la même instance 'bot' que dans Hoshimi.py - vérifie qu'elle existe
try:
    bot  # pragma: no cover
except NameError:
    raise RuntimeError("La variable 'bot' n'est pas définie dans Hoshimi.py. Assure-toi que 'bot' est déclaré avant ce bloc.")

_GPT_CONFIG_FILE = "data/gpt_hoshimi_config.json"
...
def _register_cmd_safe(name):
    return name not in getattr(bot, "all_commands", {})

# +aurole
if _register_cmd_safe("aurole"):
    @bot.command(name="aurole")
    @commands.has_permissions(manage_roles=True)
    async def _gpt_aurole(ctx, role: discord.Role):
        _gpt_config["auto_role_id"] = role.id
        _gpt_save_config(_gpt_config)
        await ctx.send(f"✅ Le rôle automatique a été défini sur {role.mention}")

# auto-role listener
async def _gpt_on_member_join(member: discord.Member):
    try:
        auto_role_id = _gpt_config.get("auto_role_id")
        if auto_role_id:
            role = member.guild.get_role(auto_role_id)
...
_existing = [h.__name__ for h in bot.listeners.get('on_member_join', [])] if getattr(bot, "listeners", None) else []
if "_gpt_on_member_join" not in _existing:
    bot.add_listener(_gpt_on_member_join, 'on_member_join')

# +createvoc
if _register_cmd_safe("createvoc"):
    @bot.command(name="createvoc")
    @commands.has_permissions(manage_channels=True)
    async def _gpt_createvoc(ctx):
        guild = ctx.guild
...
# auto temporary voice creation listener
async def _gpt_auto_temp_vc(member, before, after):
    try:
        if after and getattr(after, "channel", None) and getattr(after.channel, "name", "").lower() == "créer un vocal":
            guild = member.guild
            category = after.channel.category
            channel_name = f"🎤 Salon de {member.display_name}"
            new_vc = await guild.create_voice_channel(name=channel_name, category=category, reason="Salon vocal temporaire créé automatiquement")
            try:
                await member.move_to(new_vc)
            except Exception as e:
                print(f"⚠️ Impossible de déplacer {member.display_name} vers {new_vc.name}: {e}")
...
# stylized help (replace existing help if possible)
try:
    bot.remove_command("help")
except Exception:
    pass

if _register_cmd_safe("help"):
    @bot.command(name="help")
    async def _gpt_help(ctx):
        embed = discord.Embed(
